...
import { PartialUser, Tokens, UserPayload } from './interfaces/interfaces';
...
export class AuthService {
  ...
  async signIn({ email, password }: LoginDto, agent: string): Promise<Tokens> {
    const user = await this.usersService.validateUser(email, password);
    return user ? await this.generateTokens(user, agent) : null;
  }
  ...
  async providerAuth(authData: PartialUser, agent: string) {
    const user = await this.signIn({ email: authData.email }, agent);
